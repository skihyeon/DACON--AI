...
class AE_molding(nn.Module):
    def __init__(self, z_dim):
        ...
        self.deconv1 = nn.ConvTranspose2d(int(z_dim/(28*40)), 128, 5, stride=2, padding=2, output_padding=1, bias=False)
    ...
    def decoder(self, x):
        ...
        x = F.elu(x)
        x = self.deconv1(x)
        x = F.interpolate(F.elu(self.dbn1(x)), scale_factor=2)
        x = self.deconv2(x)
        x = F.interpolate(F.elu(self.dbn2(x)), scale_factor=2)
        x = self.deconv3(x)
        x = F.interpolate(F.elu(self.dbn3(x)), scale_factor=2)
        x = self.deconv4(x)
